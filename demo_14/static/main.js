...
// 页面状态控制
var hasList = true;  // 是否显示列表
var flag = 0;        // 0为正常地图，1为热力图
var mapWidth = 0;    // 地图框宽度
...
function searchAction(keyword, t_page) {
    ...
    $.getJSON(url, {
        ...
        'page_index' : page,
        'page_size'  : 10,
        'ak'         : '6aeacd8d328a738cfbd9569e67910aa0'  //用户ak
    },function(e) {
...
function renderMap(res, page) {
    ...
    $.each(content, function(i, item){
        ...
        function showInfo() {
            var content = "<img src='"
                        + item.weibo_pic
                        + "' style='width:111px;height:83px;float:left;margin-right:5px;' />"
                        +"<p>名称：" + item.title + "</p>"
                        +"<p>地址：" + item.address + "</p>" ;
